from planning import application, globals, urls, imports, functions, dicts
...
def nsw_planning():
    ...
    response = imports.requests.get(urls.arcgis_geocoder_url, params=params)
    ...
    gh_raster_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\image.ghx")
    gh_topography_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\topography.ghx")
    gh_buildings_elevated_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\elevate_buildings.ghx")
    ...
    rhinoutils.add_native(urls.native_url, native_post, native_layerIndex,
                          planning_model, transformer=globals.transformer2)

    # Roads
    rhinoutils.add_roads_to_model(
        tiles, zoom, road_layerIndex, planning_model, transformer=globals.transformer2)

    # Isochrone processing
    longitude_iso = lon
    latitude_iso = lat

    iso_url_w = f'https://api.mapbox.com/isochrone/v1/{globals.profile1}/{longitude_iso},{
        latitude_iso}?contours_minutes=5&polygons=true&access_token={globals.mapbox_access_token}'
    iso_url_c = f'https://api.mapbox.com/isochrone/v1/{globals.profile2}/{longitude_iso},{
        latitude_iso}?contours_minutes=10&polygons=true&access_token={globals.mapbox_access_token}'
    iso_url_d = f'https://api.mapbox.com/isochrone/v1/{globals.profile3}/{longitude_iso},{
        latitude_iso}?contours_minutes=15&polygons=true&access_token={globals.mapbox_access_token}'
    ...
    rhinoutils.add_isochrone_to_model(
        walking_data, walking_layerIndex, planning_model, transformer=globals.transformer2)

    # Cycling Curves
    rhinoutils.add_isochrone_to_model(
        cycling_data, cycling_layerIndex, planning_model, transformer=globals.transformer2)

    # Driving Curves
    rhinoutils.add_isochrone_to_model(
        driving_data, driving_layerIndex, planning_model, transformer=globals.transformer2)
    ...
    mesh_geo_list = mapboxfetcher.mapbox_topo(topography_data, 'elevation', contours_layer_EIndex,
                                              planning_model, globals.transformer2, lon, lat, gh_topography_decoded, topography_layerIndex)

    mapboxfetcher.mapbox_elevated(tiles, zoom, building_layer_EIndex, boundary_layerEIndex,
                                  bound_curve, gh_buildings_elevated_decoded, planning_model, globals.transformer2, mesh_geo_list)
    ...
    file_path = imports.os.path.abspath(
        imports.os.path.join('planning', 'tmp', 'files', filename))
...
def vic_planning():
    ...
    gh_admin_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\admin.ghx")
    gh_zoning_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\vic_qld_zoning.ghx")
    gh_bushfire_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\bushfire.ghx")
    gh_roads_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\roads.ghx")
    gh_raster_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\image.ghx")
    gh_topography_decoded = geoutils.encode_ghx_file(
        r"planning\\gh_scripts\\topography.ghx")
...
def speckleSend():
    ...
    # roads
    rhinoutils.add_roads_to_model(
        tiles, zoom, gh_roads_decoded, road_layerIndex, speckle_model, transformer=globals.transformer2)
    ...
    speckle_model.Write('planning\\tmp\\files\\' + str(filename), 7)
    ...
    file_path = 'planning\\tmp\\files\\' + filename

    rhinoutils.speckle(file_path, new_stream_id)

    stream_url = 'https://app.speckle.systems/projects/' + str(new_stream_id)
